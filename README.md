...
3. Edit [config/palindrome.conf](config/palindrome.conf) to add/enable/disable features. Edit [config/palindrome.keymap](config/palindrome.keymap) to change the keymap.
...
You can add the Palindrome zmk config repository as a module by modifying your `config/west.yml` file.
...
After making this change, add a copy of [the configuration file](config/palindrome.conf) and [the keymap](config/palindrome.keymap) to the `config` folder that is already in your repo.
...
#### Forget the Bluetooth connection on both the Palindrome and the host, then try re-pairing.
